...
class Grid_Advanced extends Grid_Basic
{
    ...
    function format_float($field)
    {
        ...
        $this->current_row[$field] =
            is_null($this->current_row[$field])
                ? '-'
                : str_replace('.',',',floatval(number_format($m, $precision)));
    ...
    function format_money($field)
    {
		...
		$this->current_row_html[$field] = '$ '.strval(number_format($this->current_row[$field], 0, ',', '.'));
    ...
    function format_boolean($field)
    {
        if ($this->current_row[$field] && $this->current_row[$field] !== 'N') {
            $this->current_row_html[$field] = '<i class="icon-check"></i>';
        } else {
            $this->current_row_html[$field] = '<i class="icon-check-empty"></i>';
        }
    }
    ...
    {
        $this->on('click','.do-'.$field)->univ()->confirm('¿Está seguro?')->ajaxec(array(
            $this->api->url(),
            $field=>$a=$this->js()->_selectorThis()->data('id'),
            $this->name.'_'.$field => $a
        ));
    ...
    function init_delete($field)
    {
        ...
        if ($id = @$_GET[$this->name.'_'.$field]) {
            ...
            // show message
            $this->js()->univ()
                ->successMessage('El borrado fue exitoso.')
                ->reload()
                ->execute();
        }
    ...
    function format_link($field)
    {
		...
		$d = date( "d", strtotime($this->current_row['Dia']));
		$m = date( "n", strtotime($this->current_row['Dia']));
		$a = date( "Y", strtotime($this->current_row['Dia']));
		$model = $this->add('Model_Empleado');
		$model->addCondition('nombre', $this->current_row['Empleado']);
		$model->tryLoadAny();
        $this->current_row['_link'] =
            /*$this->api->url($page, array($attr =>
                ...
                : $this->current_id));*/
			'mailto:'.$model['email'].'?Subject=Recordatorio de carga'.'&Body=Estimado '.$model['nombre'].
			',%0D%0AMe comunico con usted para recordarle que aún no cargó el total de su dedicación de la semana del '.
			$d.'/'.$m.'/'.$a.'.
			 Por este motivo le solicito que no se olvide de hacerlo lo antes posible.%0D%0AMuchas gracias.%0D%0A%0D%0ASaludos, Javier.';
