...
class Auth_Basic extends AbstractController {
    ...
    function createForm($page){
        ...
        $form->addField('Password','password',/*$password*/'Clave');
        $form->addSubmit('Ingresar')->addClass('atk-jackscrew')->addClass('atk-swatch-green');
    ...
    function showLoginForm(){
        ...
        if($f->isSubmitted()){
            ...
            $f->getElement('password')->displayFieldError('Email y/o clave incorrectos');
        }
		$p->add('HR');
		$p->add('H5')->set('© 2016 MVC')->addStyle('text-align', 'right')->addStyle('margin-top', '-0.5cm');
        return $p;
    }
    /** Do not override this function. */
