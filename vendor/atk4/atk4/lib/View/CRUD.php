...
class View_CRUD extends View
{
    ...
    protected function configureAdd($fields)
    {
        ...
        if ($this->isEditing('add')) {
            ...
            $this->form->addSubmit('Dar de alta');
        ...
        $this->add_button->js('click')->univ()
            ->frameURL(
                $this->api->_(
                    ...
                    ? 'New Record'
                    : 'Dar de alta '.strtolower($this->entity_name)
                ),
        ...
        if ($this->entity_name !== false) {
            $this->add_button->setHTML('<i class="icon-plus"></i> Dar de alta '.htmlspecialchars(strtolower($this->entity_name)));
        }
    }
    ...
    protected function configureEdit($fields)
    {
        ...
        $this
            ->virtual_page
            ->addColumn('edit', 'Editar '.strtolower($this->entity_name), array('descr'=>'Editar','icon'=>'pencil'), $this->grid)
            ;
    }
    ...
    {
        $this->grid->addColumn('delete', 'delete', array('icon'=>'trash', 'descr'=>'Borrar'));
    }
