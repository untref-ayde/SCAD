...
<?php
    ...
    function outputWarning($caller,$msg,$shift=0){
        ...
        if($this->log_output){
            $this->logLine($this->txtLine("aviso: $msg",$frame),'warning','error');
        }
        if(!$this->web_output){
            return true;
        }else{
    ...
    function htmlLine($msg,$frame=array(),$prefix=null){
        if(!$frame){
            return "<font style='font-family: verdana;  font-size:10px'><font color=blue>aviso: </font> <font color=red><b>$msg</b></font></font><br>";
        }else{
