...
class Export_Parser_XLS extends Export_Parser_Generic {
    ...
    function parse($captions, $data){
		...
		/*$this->output = 'header ( "Content-type: application/vnd.ms-excel" )';
		$this->output .= 'header ( "Content-Disposition: attachment; filename=export.xls" )';*/
        $this->output = "<table>";
        foreach ($data as $row){
            $cols = array();
            foreach ($row as $col){
